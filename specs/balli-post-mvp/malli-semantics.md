...
- `validate` on `:=>`/`:function` = `(fn? x)` / `ifn?`-equivalent (python/callable), UNLESS a function-checker is in effect — then generative check: generate N (default 100) input seqs from input-seqex generator, apply, validate outputs; valid iff no failure. Balli checker scope: the `:balli.core/function-checker` opt applies to RAW-form `validate`/`explain` calls (bypassing the global cache) and to `schema` construction (baked into the object); **call-time opts on an already-constructed schema object are IGNORED** (MVP opts rule — the object's baked config wins).
