...
- [ ] Refs: `$ref` to `#/definitions/<name>` (name = `(subs (str kw) 1)`); definitions atom collected during transform; recursion stopper before recursing into a ref target; emit top-level `"definitions"` only when non-empty
...
- [ ] `compile-parser [ast registry]` + `compile-unparser` in compile.lpy: simple types → validate-then-value; `:orn` → Tag of first match; `:multi` → Tag(dispatch, parsed); `:maybe`/`:and` (first-child parse, validate rest)/`:or` (first match, untagged)/colls (per-element)/`:map` (entry values; Tag/Tags guard)/`:tuple`/`:map-of`/`:ref`
...
- [ ] Validate: `python/callable`; with opts `{:balli.core/function-checker checker}` → generative (checker from `balli.generator/function-checker`: N=100 generated input seqs → apply → validate output; explain carries `:balli.core/invalid-output` + failing pair)
