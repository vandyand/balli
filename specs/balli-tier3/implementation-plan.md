...
- [ ] generator: pred→schema mapping table per §4 (unmappable → no-generator); comparators (int-based above/below child; `:not=` filter)
- [ ] json-schema: preds → obvious types or `{}`; comparators → exclusiveMinimum/minimum/exclusiveMaximum/maximum/`{}`
- [ ] transform: preds/comparators pass-through (int? does NOT get string-decode — matches malli where predicate schemas DO have coders... CHECK malli transform.cljc `-string-decoders` includes `'int?` etc. → YES, malli string-decodes predicate schemas. Balli: wire pred types into string/json coder tables for int?/integer?/pos-int?/neg-int?/nat-int?/double?/float?/number?/boolean?/keyword?/symbol?/uuid?)
...
- [ ] `balli.core`: raw-form cache invalidation — `set-default-registry!`/`register!` clear the global raw cache and bump the default token (registry-tokens pattern); document snapshot semantics for baked schema objects (docstring + README later)
...
- [ ] `src/balli/time.lpy`: type table `{:time/instant {:check <fn> :decode <fn> :encode <fn> :compare native :gen-default [lo hi]} ...}` for the 5 types; ISO-duration regex parser/formatter (per spike findings, incl. negative + fractional); `time-transformer` (name `:time`) with decoders/encoders per table
