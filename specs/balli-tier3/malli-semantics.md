...
- Transformers: extend BOTH `string-transformer` and `json-transformer` decoders/encoders with the `:time/*` entries (malli ships a separate `time-transformer` to compose; Balli: add a `time-transformer` in `balli.time` AND fold time coders into the built-ins? NO — mirror malli: separate `(bt-time/time-transformer)` users compose via `(bt/transformer (bt/json-transformer) (btime/time-transformer))`). Decoders lenient (parse failure → input unchanged). Encoders emit strings.
...
- Caching hazard: balli.core's global raw-form cache keys include a registry token via `identical?`; a MUTATED default registry must not serve stale compiled fns → `set-default-registry!`/`register!` must invalidate (clear) the raw-form cache AND bump the default-registry token. Schema OBJECTS keep their construction-time snapshot (documented: objects are immutable snapshots; malli's mutable registry is also lookup-time for raw forms but baked objects keep resolved refs — Balli matches by clearing raw cache only).
