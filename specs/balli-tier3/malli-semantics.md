...
- Transformers: extend BOTH `string-transformer` and `json-transformer` decoders/encoders with the `:time/*` entries (malli ships a separate `time-transformer` to compose; Balli: add a `time-transformer` in `balli.time` AND fold time coders into the built-ins? NO — mirror malli: separate `(bt-time/time-transformer)` users compose via `(bt/transformer (bt/json-transformer) (bt/time-transformer))`). Decoders lenient (parse failure → input unchanged). Encoders emit strings.
